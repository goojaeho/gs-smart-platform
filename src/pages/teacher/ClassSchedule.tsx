    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-gray-900">수업 일정</h2>
            <p className="text-gray-600 mt-1">주간 수업 및 멘토링 일정</p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="flex items-center justify-between">
            ...
            <h3 className="text-lg font-bold text-gray-900">
              2024년 1월 4주 ({currentWeek.toLocaleDateString('ko-KR')} - {new Date(currentWeek.getTime() + 6 * 24 * 60 * 60 * 1000).toLocaleDateString('ko-KR')})
            </h3>
            <button
              onClick={() => setCurrentWeek(new Date(currentWeek.getTime() + 7 * 24 * 60 * 60 * 1000))}
              className="p-2 hover:bg-gray-100 rounded"
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {schedules.map(day => (
                    <th key={day.id} className="px-4 py-3 text-center text-sm font-medium text-gray-700 min-w-[150px]">
                      <div>{day.day}요일</div>
              ...
              <tbody>
                {['09:00', '10:00', '13:00', '14:00', '15:00', '16:00'].map(hour => (
                  <tr key={hour} className="border-t">
                    <td className="px-4 py-2 text-sm text-gray-600">{hour}</td>
                    {schedules.map(day => {
                      ...
                      if (slot && shouldShow) {
                        return (
                          <td key={day.id} className="px-2 py-2">
                            <div className="p-2 rounded-lg bg-blue-50 border border-blue-200 cursor-pointer hover:bg-blue-100">
                              <div className="flex items-center justify-between mb-1">
                                <span className="font-medium text-xs text-gray-900">{slot.subject}</span>
                                <span className={`px-1 py-0.5 rounded text-xs ${getTypeColor(slot.type)}`}>
                                  {slot.type === 'online' ? '온라인' :
                                   slot.type === 'offline' ? '오프라인' :
                                   slot.type === 'phone' ? '전화' : slot.type}
                                </span>
                              </div>
                              <p className="text-xs text-gray-700 mb-1">{slot.topic}</p>
                      ...
                      return <td key={day.id} className="px-2 py-2"></td>;
                    })}
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-bold text-gray-900 mb-4">오늘의 수업</h3>
